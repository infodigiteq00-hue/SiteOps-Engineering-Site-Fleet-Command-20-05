...
import {
  ...
  countEffectiveMachineryUnits,
  summarizeMachineryDisplayQty,
...
describe("summarizeMachineryDisplayQty", () => {
  it("shows entered qty and unit type for a single-unit measured row", () => {
    const machines = [
      machine({
        id: "h1",
        code: "HOS-001",
        name: "Hose pipe 5 inch dia",
        category: "Hose pipe 5 inch dia",
        unitType: "metre",
        stockQuantity: 50,
      }),
    ];
    expect(countEffectiveMachineryUnits(machines)).toBe(50);
    expect(summarizeMachineryDisplayQty(machines)).toEqual({ qty: 50, unitLabel: "metre" });
  });

  it("keeps piece units as stock qty with their unit type", () => {
    const machines = [
      machine({ id: "a", code: "P-001", name: "Pulley 1", stockQuantity: 1 }),
      machine({ id: "b", code: "P-002", name: "Pulley 2", stockQuantity: 1 }),
    ];
    expect(summarizeMachineryDisplayQty(machines)).toEqual({ qty: 2, unitLabel: "nos" });
  });
});
