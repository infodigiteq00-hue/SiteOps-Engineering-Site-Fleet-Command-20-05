...
export function groupSiteMachinery(machines: Machine[], siteId: string): SiteClosureGroup[] {
  ...
  return Array.from(grouped.entries()).map(([key, entry]) => ({
    ...
    units: entry.units,
    count: usesContinuousQuantity(entry.unitType)
      ? 1
      : entry.machineIds.reduce((sum, machineId) => {
          const machine = atSite.find((m) => m.id === machineId);
          return sum + (machine ? machineryStockQuantity(machine) : 1);
        }, 0),
  }));
}

/** On-hand quantity for a machinery row (defaults to 1). */
export function machineryStockQuantity(machine: Pick<Machine, "stockQuantity">): number {
  const qty = machine.stockQuantity ?? 1;
  return Number.isFinite(qty) && qty > 0 ? qty : 1;
}

/** Sum on-hand stock qty so a single-unit 50 metre row displays as 50, not 1. */
export function countEffectiveMachineryUnits(machines: Machine[]): number {
  return machines.reduce((sum, machine) => sum + machineryStockQuantity(machine), 0);
}

/** Left-side qty badge: entered number + the chosen unit type when the group is uniform. */
export function summarizeMachineryDisplayQty(machines: Machine[]): { qty: number; unitLabel: string } {
  const qty = countEffectiveMachineryUnits(machines);
  if (machines.length === 0) return { qty: 0, unitLabel: "Machinery" };
  const units = new Set(machines.map((machine) => normalizeMachineryUnitType(machine.unitType)));
  if (units.size === 1) {
    const [unitLabel] = units;
    return { qty, unitLabel: unitLabel ?? "Machinery" };
  }
  return { qty, unitLabel: "Machinery" };
}
